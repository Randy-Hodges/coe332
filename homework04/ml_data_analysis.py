...
def main():
    ...
    [v, d] = [len(ml_data['meteorite_landings']), compute_average_mass(ml_data['meteorite_landings'], 'mass (g)')] 
    print(f'Average Mass of {v} meteor(s): \n {d}') # bruh fstrings not working idk man just that kinda day
    ...
    for row in ml_data['meteorite_landings']:
        # increase the hemispheres data by one based on which entry a particular meteorite is in
        hemispheres[check_hemisphere(float(row['reclat']), float(row['reclong']))] += 1

    # Print hemisphere data
    print('\nHemisphere summary data:')
    for hemisphere, amount_found in hemispheres.items():
        print(f'There were {amount_found} meteors found in the {hemisphere} quadrant')

    # Print class data
    print('\nClass summary data:')
    class_dict = count_classes(ml_data['meteorite_landings'], 'recclass')
    for key, value in class_dict.items():
        print(f'The {key} class was found {value} time(s)')
